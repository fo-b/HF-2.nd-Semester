    ...
    for sqs_message in sqs_messages:
        ...
        if state.lower() == "stopped":
            ...
            response = ec2.describe_instances(
                Filters=[
                    {'Name': 'tag:orderID', 'Values': [process_instance_id]},
            ...
            if instances:
                ...
                print(f"No running EC2 instance found with orderID {process_instance_id}.")
                return {
                    'statusCode': 404,
                    'body': f'No running EC2 instance found with orderID {process_instance_id}.'
                }
        ...
        if state.lower() == "deleted":
            ...
            if instances:
                ...
                print(f"No running instances found with orderID {process_instance_id}. Nothing to delete.")
                return {
                    'statusCode': 404,
                    'body': f'No running instances found with orderID {process_instance_id}. Nothing to delete.'
                }
        
        # Check if the state is "restarted"
        if state.lower() == "restarted":
            # If the state is "restarted," find the EC2 instance with the given orderID
            ec2 = boto3.client('ec2')
            response = ec2.describe_instances(Filters=[
                {'Name': 'tag:orderID', 'Values': [process_instance_id]}
            ])
        
            # Check if there are running instances with the given orderID
            instances = response.get('Reservations', [])
            if instances:
                ...
                print(f"No running instances found with orderID {process_instance_id}. Nothing to restart.")
                return {
                    'statusCode': 404,
                    'body': f'No running instances found with orderID {process_instance_id}. Nothing to restart.'
                }
                
        # Check if the state is "start"
        if state.lower() == "start":
            # If the state is "start," find the EC2 instance with the given orderID
            ec2 = boto3.client('ec2')
            response = ec2.describe_instances(Filters=[
                {'Name': 'tag:orderID', 'Values': [process_instance_id]}
            ])
    
            # Check if there are stopped instances with the given orderID
            instances = response.get('Reservations', [])
            if instances:
                instance_id_to_start = instances[0]['Instances'][0]['InstanceId']
    
                # Start the specified EC2 instance
                ec2.start_instances(InstanceIds=[instance_id_to_start])
    
                print(f"EC2 instance {instance_id_to_start} is being started.")
                return {
                    'statusCode': 200,
                    'body': f'EC2 instance {instance_id_to_start} is being started.'
                }
            else:
                print(f"No stopped instances found with orderID {process_instance_id}. Nothing to start.")
                return {
                    'statusCode': 404,
                    'body': f'No stopped instances found with orderID {process_instance_id}. Nothing to start.'
                }
